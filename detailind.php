...
$empresaava = isset($_GET['empresaava']) ? $_GET['empresaava'] : null;
$matriculaava = isset($_GET['matriculaava']) ? $_GET['matriculaava'] : null;
$empresa = isset($_GET['empresa']) ? $_GET['empresa'] : null;
$matricula = isset($_GET['matricula']) ? $_GET['matricula'] : null;
$tipava = isset($_GET['tipava']) ? $_GET['tipava'] : null;
...
if (empty($matriculaava)) {
    echo $v->validarCampo("Matricula", $matriculaava, 10, 1);
} else {
    echo $v->validarNumero("Matrícula", $matriculaava);
}
...
//    CALCULA MEDIA
    ...
    if (isset($mediafun)) {
        ...
        echo "<script> document.location = 'index.php?var=4' </script>";
    }
